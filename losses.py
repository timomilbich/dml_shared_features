...
def cluster_and_pca(opt, dataloader, model, mode='mean_std', feat='embed_res', plot=False, verbose=False, epoch=-1):
    #Compute features
    _ = model.eval()
    with torch.no_grad():
        features = []
    ...
    # get cluster
    clusters = [np.where(cluster_assignments == k)[0] for k in range(opt.num_cluster)]

    # compute pcas
    from sklearn.decomposition import PCA
    Ws = list()
    mus = list()
    for i in range(opt.num_cluster):
        pca = PCA(n_components=opt.dim_pca)
        pca.fit(features[clusters[i], :])
        Ws.append(pca.components_)
        mus.append(pca.mean_)

        print('Explained var = {}'.format(np.sum(pca.explained_variance_ratio_[0:opt.dim_pca])))


    return cluster_assignments, Ws, mus


def fit_local_pca(opt, dataloader, model, mode='mean_std', feat='embed_res', niter=1, plot=False, verbose=False, epoch=None):
    #Compute features
    _ = model.eval()
    with torch.no_grad():
        features = []
    ...
    # n_comps = 20
    # dim_subspace = 64 # 64
    # local_pca = Local_PCA(n_subspaces=n_comps, dim_subspace=dim_subspace, max_iter=10, init='random', robust=False)
    # local_pca.perform_local_pca(features)
    # print('Training time: {}'.format(time.time() - end))

    # fit mppca
    end = time.time()
    n_comps = opt.num_cluster
    pi_init, mu_init, W_init, sigma2_init, _ = pca_initialization_kmeans(features, n_comps, opt.dim_mppca, variance_level=None, kmeans=True) # 1000
    pi_new, mu_new, W_new, sigma2_new, R, L, sigma2hist = mppca_gem(features, pi_init, mu_init, W_init, sigma2_init, niter=opt.mppca_iter, verbose=verbose)
    assignments = np.argmax(R, axis=1)
    print('Training time: {}'.format(time.time() - end))

    # # plot subspace member
    # assignments_per_subspace = [np.where(assignments == k)[0] for k in range(n_comps)]
    # gt_label = [dataloader.dataset.image_list[i][1] for i in range(len(dataloader.dataset))]
    # purities = list()
    # for k in range(n_comps):
    #
    #     member = assignments_per_subspace[k]
    #     if verbose: print('# member of subspace = {}'.format(len(member)))
    #     if len(member) == 0: continue
    #
    #     # member = local_pca.R[k]['sample_ids_trained']
    #
    #     # compute subspace purity
    #     purity_cluster = compute_cluster_purity(member, gt_label)
    #     purities.append(round(purity_cluster[0], 3))
    #     if verbose: print('*** subspace purity: {}'.format(round(purity_cluster[0], 3)))
    #
    #     # plot cluster
    #     perm = np.random.permutation(len(member))
    #     member = [member[k] for k in perm]
    #
    #     # save result
    #     path_save = opt.save_path + '/subspaces'
    #     if epoch is not None: path_save += '/epoch_{}'.format(epoch)
    #     if not os.path.isdir(path_save): os.makedirs(path_save)
    #     path_save += '/space_{}.pdf'.format(k)
    #     visualize_single_cluster(member, dataloader.dataset.image_list,
    #                              subtitles=None, n_max_samples=100,
    #                              path_save=path_save, plot=plot,
    #                              title='purity_cluster: {}; #member: {}'.format(round(purity_cluster[0], 3), len(member)))
    #
    # print('*** Mean cluster purity: {}'.format(np.mean(purities)))


    # # get explained variance per dim and subspace
    # from sklearn.decomposition import PCA
    #
    # # joint summary
    # import matplotlib.pyplot as plt
    # f, ax = plt.subplots(1, 1, figsize=(5, 5))
    # f.suptitle('pooled layer2 (normed+standardized)')
    #
    # assignments_per_subspace = [np.where(assignments == k)[0] for k in range(n_comps)]
    # for i in range(n_comps):
    #
    #     #get members
    #     member = assignments_per_subspace[i]
    #
    #     # compute pca
    #     n_pca_dims = opt.dim_mppca
    #     pca = PCA(n_components=n_pca_dims)
    #     pca.fit(features[member, :])
    #     energy_per_k = [np.sum(pca.explained_variance_ratio_[0:k]) for k in range(n_pca_dims)]
    #
    #     # plot
    #     ax.plot(energy_per_k)
    #
    # ax.set(xlabel='n_components', ylabel='accumulated energy')
    # ax.set_title('PCA energy')
    # plt.show(False)
    # f.savefig('./Analysis/mppca20_64_iter3_lay2_stand_norm.pdf')
    # plt.close()

    # # project to dims and plot
    # assignments_per_subspace = [np.where(assignments == k)[0] for k in range(n_comps)]
    # n_top_comps = 5
    # for i in range(n_comps):
    #
    #     # get members
    #     member = assignments_per_subspace[i]
    #     features_member = features[member, :]
    #
    #     for j in range(n_top_comps):
    #
    #         # project onto top comps
    #         feat_project = list()
    #         for k in range(len(member)):
    #             feat_project.append(np.dot(W_new[i, :, j], features_member[k, :] - mu_new[i, :]))
    #
    #         # sort samples w.r.t. projection
    #         ids_sort = np.argsort(feat_project)
    #
    #         # plot sorted samples
    #         visualize_single_cluster([member[k] for k in ids_sort], dataloader.dataset.image_list,
    #                                  subtitles=None, n_max_samples=100,
    #                                  path_save=None, plot=plot,
    #                                  title='subspace {}; axis {}'.format(i, j))

    # # analyse rank of stacked subspaces
    # subspaces_stacked = W_new[0, :, :].transpose()
    # for i in range(1, W_new.shape[0]):
    #     subspaces_stacked = np.concatenate((subspaces_stacked, W_new[i, :, :].transpose()), axis=0)
    #
    # rank = np.linalg.matrix_rank(subspaces_stacked, tol=0.1)
    # print('Rank of subspaces = {}'.format(rank))
    #
    # import math
    ...
    #
    # angles = list()
    # angle_ids1 = np.random.choice(subspaces_stacked.shape[0], size=30)
    # angle_ids2 = np.random.choice(subspaces_stacked.shape[0], size=30)
    # for i in angle_ids1: # subspaces_stacked.shape[0]
    #     for j in angle_ids2: # i+1, subspaces_stacked.shape[0]
    #         angles.append(angle(subspaces_stacked[i, :], subspaces_stacked[j, :]))
    #
    # f, ax = plt.subplots(1, 1, figsize=(5, 5))
    # f.suptitle('angle distribution: pooled layer2 (non-standardized)')
    # ax.hist(angles, bins=360)
    # ax.set(xlabel='angles (radian)', ylabel='occurences')
    # ax.set_title('angle distribution')
    # plt.show(True)
    # # f.savefig('./Analysis/mppca20_64_iter3_lay2_stand_norm.pdf')
    # plt.close()

    # # analyse distances between subspace origins
    # from sklearn.metrics import pairwise_distances
    # dists_between_subspaces = pairwise_distances(mu_new, mu_new)
    # dists_between_features = pairwise_distances(features, features)
    # print('max. distance samples = {}'.format(np.max(dists_between_features)))
    # print('min. distance samples = {}'.format(np.min(dists_between_features)))
    # print('mean. distance samples = {}'.format(np.mean(dists_between_features)))
    #
    # print('max distances between subspaces = {}'.format(np.max(dists_between_subspaces)))
    # print('min distances between subspaces = {}'.format(np.min(dists_between_subspaces)))
    # print('mean distances between subspaces = {}'.format(np.mean(dists_between_subspaces)))

    # cluster within subspaces
    gt_label = [dataloader.dataset.image_list[i][1] for i in range(len(dataloader.dataset))]
    assignments_per_subspace = [np.where(assignments == k)[0] for k in range(n_comps)]
    n_cluster_subspace = 5
    for i in range(opt.num_cluster):
        print('*** subspace id: {}'.format(i))

        member = assignments_per_subspace[i]
        features_member = features[member, :]

        # project
        feat_project = np.zeros([len(member), opt.dim_mppca], dtype=np.float32)
        W_tmp = W_new[i, :].transpose()
        for k in range(len(member)):
            feat_tmp = features_member[k, :] - mu_new[i, :]
            feat_project[k, :]  = np.dot(W_tmp, feat_tmp)

        # cluster
        n_samples, dim = feat_project.shape
        kmeans = faiss.Kmeans(dim, n_cluster_subspace)
        kmeans.n_iter, kmeans.min_points_per_centroid, kmeans.max_points_per_centroid = 20, 5, 1000000000

        kmeans.train(feat_project)
        _, cluster_assignments_subspace = kmeans.index.search(feat_project, 1)

        # plot cluster
        assignments_per_clust = [member[np.where(cluster_assignments_subspace == k)[0]] for k in range(n_cluster_subspace)]
        for j in range(n_cluster_subspace):
            print('cluster id: {}'.format(j))

            member_sub = assignments_per_clust[j]

            # compute cluster purity
            purity_cluster = compute_cluster_purity(member_sub, gt_label)
            print('cluster purity: {}'.format(round(purity_cluster[0], 3)))

            # plot
            visualize_single_cluster(member_sub, dataloader.dataset.image_list,
                                     subtitles=None, n_max_samples=100,
                                     path_save=None, plot=True,
                                     title='purity_cluster: {}; #member: {}'.format(round(purity_cluster[0], 3), len(member_sub)))


    return assignments


def fit_local_pca_and_cluster(opt, dataloader, model, mode='mean_std', feat='embed_res', verbose=False):
    #Compute features
    _ = model.eval()
    with torch.no_grad():
        features = []
    ...
    n_comps = opt.num_subspaces
    pi_init, mu_init, W_init, sigma2_init, _ = pca_initialization_kmeans(features, n_comps, opt.dim_mppca, variance_level=None, kmeans=True) # 1000
    pi_new, mu_new, W_new, sigma2_new, R, L, sigma2hist = mppca_gem(features, pi_init, mu_init, W_init, sigma2_init, niter=opt.mppca_iter, verbose=verbose)
    assignments_tmp = np.argmax(R, axis=1)
    print('Training time: {}'.format(time.time() - end))

    # cluster within subspaces
    assignments_per_subspace = [np.where(assignments_tmp == k)[0] for k in range(n_comps)]
    cluster_per_subspace = list()
    for i in range(n_comps):
        ...
        # cluster
        n_samples, dim = feat_project.shape
        kmeans = faiss.Kmeans(dim, opt.num_clust_per_subspace)
        kmeans.n_iter, kmeans.min_points_per_centroid, kmeans.max_points_per_centroid = 20, 5, 1000000000

        kmeans.train(feat_project)
        _, cluster_assignments_subspace = kmeans.index.search(feat_project, 1)
        cluster_per_subspace.extend([member[np.where(cluster_assignments_subspace == k)[0]] for k in range(opt.num_clust_per_subspace)])

        # plot cluster
        gt_label = [dataloader.dataset.image_list[i][1] for i in range(len(dataloader.dataset))]
        assignments_per_clust = [member[np.where(cluster_assignments_subspace == k)[0]] for k in range(opt.num_clust_per_subspace)]
        for j in range(opt.num_clust_per_subspace):
            print('cluster id: {} - {}'.format(i,j))
    ...
    assignments = np.zeros_like(assignments_tmp)
    surrogate_label_curr = 0
    for clust in cluster_per_subspace:
        assignments[clust] = surrogate_label_curr
        surrogate_label_curr += 1

    return assignments


def find_svm_projections(opt, dataloader, model, mode='mean_std', feat='embed_res', visualize=False):
    ...
    N_samples, ndim = features.shape
    min_svm_prob_init = 0.6 # 0.6
    min_svm_prob_refine = 0.4 # 0.5
    class_ratio = 20
    n_pos = 5
    n_neg = class_ratio * n_pos
    attr_min_size = 20
    neg_mode = 'rand'
    n_seq_cut = 100 #200

    # get classes
    gt_labels = [dataloader.dataset.image_list[i][1] for i in range(len(dataloader.dataset))]

    # build faiss
    index = faiss.IndexFlatL2(ndim)  # build the index
    index.add(features)  # add vectors to the index

    # find knn
    D, I = index.search(features, n_pos * 10)

    # train classifier
    print('*** Training classifier: \n'
          'neg_mode [{}] \n'
          '# pos = {} \n'
          '# negs = {}'.format(neg_mode, n_pos, n_neg))

    cand_pool = np.asarray((range(N_samples)))
    groups_attr = list()
    count_attr = 0
    n_iter_wo_add = 0
    end = time.time()
    while len(cand_pool) > 0:

        if count_attr == opt.num_cluster:
            break

        if len(cand_pool) % 100 == 0:
            print('{} cands left'.format(len(cand_pool)))

        # get positives
        id_anchor = np.random.choice(cand_pool, 1)[0]
        ids_pos = [I[id_anchor][k] for k in range(len(I[id_anchor])) if gt_labels[I[id_anchor][k]] != gt_labels[I[id_anchor][0]]] # I[id_anchor]
        ids_pos = [id_anchor] + ids_pos[1:n_pos] #  ids_pos[0:n_pos]
        y_pos = np.ones(n_pos, dtype=int)

        # ensure sufficient number of positives to learn classifier
        if len(ids_pos) < n_pos:
            continue

        # get negatives
        if neg_mode == 'rand': # get negatives - random
            ids_neg = np.random.choice([idx for idx in range(N_samples) if idx not in ids_pos], n_neg, replace=False)
            y_neg = np.zeros(n_neg, dtype=int)
        elif neg_mode == 'same_class': # get negatives - positive gt classes
            pos_labels = [gt_labels[k] for k in ids_pos]
            ids_neg = np.asarray([idx for idx in range(N_samples) if gt_labels[idx] in pos_labels and idx not in ids_pos])
            ids_neg = np.random.choice(ids_neg, n_neg, replace=False)
            y_neg = np.zeros(len(ids_neg), dtype=int)

        # train
        clf = svm.SVC(kernel='linear', class_weight={1: 1}, probability=True)
        clf.fit(np.concatenate([features[ids_pos, :], features[ids_neg, :]], axis=0), np.concatenate([y_pos, y_neg], axis=0))

        # apply on samples
        probs = clf.predict_proba(features[cand_pool, :])
        good = probs[:, 1] > min_svm_prob_init
        ids_good = cand_pool[good]

        # add attribute
        if len(ids_good) > attr_min_size:

            purity_cluster = compute_cluster_purity(ids_pos, gt_labels)
            print('initial attr purity: {}'.format(round(purity_cluster[0], 3)))

            # refine classifier
            y_pos = np.ones(len(ids_good), dtype=int)
            n_neg_refine = len(ids_good) * 10 # 200
            ids_neg = np.random.choice([idx for idx in range(N_samples) if idx not in ids_pos],
                                       n_neg_refine, replace=False)
            y_neg = np.zeros(n_neg_refine, dtype=int)
            clf.fit(np.concatenate([features[ids_good, :], features[ids_neg, :]], axis=0),
                    np.concatenate([y_pos, y_neg], axis=0))

            # apply on samples
            probs = clf.predict_proba(features[cand_pool, :])
            good = probs[:, 1] > min_svm_prob_refine
            probs_good = probs[good, 1]
            ids_good = cand_pool[good]

            # add samples
            print('{} attribute samples have been added. [total: {}] [{}]'.format(
                len(ids_good), np.sum([len(attr) for attr in groups_attr]), count_attr))
            groups_attr.append(ids_good)

            # compute cluster purity
            purity_cluster = compute_cluster_purity(ids_good, gt_labels)
            print('attr purity: {} \n'.format(round(purity_cluster[0], 3)))

            # plot
            if visualize:
                dists_pos = D[id_anchor]
                dists_pos = dists_pos[0:n_pos]
                visualize_single_cluster(ids_pos, dataloader.dataset.image_list, subtitles=None, n_max_samples=100, path_save=None, plot=False, close_fig=False)

                probs_good = [round(prob, 2) for prob in probs_good]
                idx_sort = np.argsort(probs_good)[::-1]
                visualize_single_cluster(ids_good[idx_sort], dataloader.dataset.image_list, subtitles=np.asarray(probs_good)[idx_sort],
                                              n_max_samples=100, path_save=None, plot=True)

            # update and reset
            cand_pool = np.setdiff1d(cand_pool, ids_good) # todo: remove only anchor instead of whole group ?
            count_attr += 1
            n_iter_wo_add = 0
        else:
            cand_pool = np.setdiff1d(cand_pool, id_anchor)
            n_iter_wo_add += 1

        if n_iter_wo_add >= n_seq_cut:
            break

    purity_per_cluster = [compute_cluster_purity(attr, gt_labels)[0] for attr in groups_attr]
    print('*** Summary [{} s] *** \n'
          '{} attributes \n'
          '{} attribute samples \n'
          'have been added! \n'
          'mean purity = {} \n'
          .format(time.time() - end, count_attr, np.sum([len(attr) for attr in groups_attr]),
          np.mean(purity_per_cluster)))


    # convert datastructure
    assignments = -1 * np.ones_like(gt_labels)
    surrogate_label_curr = 0
    for attr in groups_attr:
        assignments[attr] = surrogate_label_curr
        surrogate_label_curr += 1
